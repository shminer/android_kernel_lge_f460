...
static inline struct lp5521_chip *engine_to_lp5521(
...
static void lp5521_led_brightness_work(struct work_struct *work)
{
	struct lp5521_led *led =
		container_of(work, struct lp5521_led, brightness_work);
	...
	ret = lp5521_write(client, LP5521_REG_LED_PWM_BASE + led->chan_nr,
			led->brightness);
...
static ssize_t store_led_ic_rw(struct device *dev,
			     struct device_attribute *attr,
			     const char *buf, size_t len)
{
	struct i2c_client *client = to_i2c_client(dev);
	...
	sscanf(buf, "%s %d %d", string, &address, &data);

	if (!strncmp(string, "read", 4)) {
		do {
		ret = lp5521_read(client, address, &value);
		} while (--data > 0 );

	} else if (!strncmp(string, "write", 4)) {
		ret = lp5521_write(client, address, data);
		ret = lp5521_read(client, address, &value);
	} else
...
#if 0
	/* this process is not need, because dummy pattern defined 
	   in board file */
	if (mode == PATTERN_FAVORITE_MISSED_NOTI
			|| mode == PATTERN_CHARGING_COMPLETE_50
			|| mode == PATTERN_CHARGING_50) {
		...
		struct device_attribute *attr, const char *buf, size_t len)
{
	...
	for (i = 0; i < LP5521_MAX_LEDS ; i++) {
		...
		LP5521_INFO_MSG("[%s] modify_current : %d", __func__,
				modify_current);
...
static DEVICE_ATTR(led_current, S_IRUGO | S_IWUSR,
		show_current, store_current);
static DEVICE_ATTR(max_current, S_IRUGO , show_max_current, NULL);
static DEVICE_ATTR(led_ic_rw, S_IRUGO | S_IWUSR, NULL, store_led_ic_rw);
...
static int lp5521_init_led(struct lp5521_led *led, struct i2c_client *client,
		...
		const struct i2c_device_id *id)
{
	...
	if (&client->dev.of_node) {
		...
		chip->rgb_led_en = of_get_named_gpio(client->dev.of_node,
				"ti,led_en", 0);
		LP5521_INFO_MSG("chip->rgb_led_en ==%d \n", chip->rgb_led_en);
		if (!gpio_is_valid(chip->rgb_led_en)) {
			pr_err("Fail to get gpio for rgb_led_en.\n");
			goto fail0;
		} else {
			...
			if (ret) {
				pr_err("requestgpio failed, rc=%d\n", ret);
	...
	for (i = 0; i < lp5521_pdata.num_channels; i++) {
		...
		lp5521_set_led_current(chip, led, chip->leds[led].led_current);
...
static int lp5521_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	LP5521_INFO_MSG("[%s] id_pattern_play = %d\n", __func__,
			chip->id_pattern_play);
	if (lge_get_board_revno() > HW_REV_EVB2) {
		if (chip->pdata->enable && chip->id_pattern_play
				== PATTERN_OFF) {
...
static int lp5521_resume(struct i2c_client *client)
{
	...
	if (lge_get_board_revno() > HW_REV_EVB2) {
		...
		if (chip->id_pattern_play == PATTERN_OFF) {
			...
			ret = lp5521_configure(client);
			if (ret < 0) {
				dev_err(&client->dev, "error config chip\n");
			}
		}
	}
