...
#include <linux/mfd/leds-max77819-flash.h>
...
static void max77819_flash_set(struct led_classdev *led_cdev,
		enum led_brightness brightness)
{
	struct max77819_flash *max77819_flash = container_of(led_cdev,
			struct max77819_flash, led_flash);
	struct regmap *regmap = max77819_flash->io->regmap;
...
static enum led_brightness max77819_flash_get(struct led_classdev *led_cdev)
{
	...
	if ((value & MAX77819_FLASH_FLED_EN) ==
			(MAX77819_OFF << M2SH(MAX77819_FLASH_FLED_EN)))
		return LED_OFF;
	...
	return (enum led_brightness)((value & MAX77819_FLASH_I)
			>> M2SH(MAX77819_FLASH_I));
}

static void max77819_torch_set(struct led_classdev *led_cdev,
		enum led_brightness brightness)
{
	struct max77819_flash *max77819_flash = container_of(led_cdev,
			struct max77819_flash, led_torch);
	struct regmap *regmap = max77819_flash->io->regmap;
...
static enum led_brightness max77819_torch_get(struct led_classdev *led_cdev)
{
	...
	if ((value & MAX77819_TORCH_FLED_EN) ==
			(MAX77819_OFF << M2SH(MAX77819_TORCH_FLED_EN)))
		return LED_OFF;
	...
	return (enum led_brightness)((value & MAX77819_TORCH_I)
			>> M2SH(MAX77819_TORCH_I));
}
		...
		struct max77819_flash_platform_data *pdata)
{
	...
	ret = regmap_write(regmap, MAX77819_TORCH_TMR, MAX77819_TORCH_TMR_DUR
			| MAX77819_DIS_TORCH_TMR | MAX77819_TORCH_TMR_MODE);
	if (IS_ERR_VALUE(ret))
		return ret;
...
#ifdef CONFIG_OF
static struct
max77819_flash_platform_data *max77819_flash_parse_dt(struct device *dev)
{
	struct max77819_flash_platform_data *pdata;
	...
	if (!of_property_read_u32(np, "maxflash-threshold",
				&pdata->maxflash_threshold))
		pdata->maxflash_threshold = 2400000;

	if (!of_property_read_u32(np, "maxflash-hysteresis",
				&pdata->maxflash_hysteresis))
		pdata->maxflash_hysteresis = 100000;

	if (!of_property_read_u32(np, "maxflash-falling-timer",
				&pdata->maxflash_falling_timer))
		pdata->maxflash_falling_timer = 256;

	if (!of_property_read_u32(np, "maxflash-rising-timer",
				&pdata->maxflash_rising_timer))
...
static int max77819_flash_probe(struct platform_device *pdev)
{
	...
	max77819_flash = devm_kzalloc(dev, sizeof(struct max77819_flash),
			GFP_KERNEL);
	if (unlikely(!max77819_flash))
		return -ENOMEM;

	// initial setup
	ret = max77819_flash_hw_setup(io, pdata);
	if (IS_ERR_VALUE(ret)) {
		dev_err(&pdev->dev, "hw init fail : %d\n", ret);
		return ret;
	}

	max77819_flash->chip = chip;
	max77819_flash->io = io;
	...
	max77819_flash->led_flash.max_brightness = MAX77819_FLASH_I
		>> M2SH(MAX77819_FLASH_I);
	...
	max77819_flash->led_torch.max_brightness = MAX77819_TORCH_I
		>> M2SH(MAX77819_TORCH_I);
...
static int max77819_flash_remove(struct platform_device *pdev)
{
