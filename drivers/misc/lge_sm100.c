...
static int vibrator_ic_enable_set(int enable, struct timed_vibrator_data *vib_data)
{
	...
	INFO_MSG("[sm100] Haptic_EN_GPIO Value : %d\n", gpio);
...
static int vibrator_pwm_set(int enable, int amp, int n_value)
{
	...
	INFO_MSG("[sm100] amp=%d, n_value=%d\n", amp, n_value);
...
static int vibrator_gpio_init(struct timed_vibrator_data *vib_data)
{
	...
	INFO_MSG("[sm100] vibrator gpio init\n");
...
static int android_vibrator_probe(struct platform_device *pdev)
{
	...
	if (vib->vpwr_on != 1) {
		if (!(vib->vreg_l19)) {
			...
			if (IS_ERR(vib->vreg_l19)) {
				ERR_MSG("[sm100] %s: regulator get of pm8941_l21 failed (%ld)\n",
						__func__, PTR_ERR(vib->vreg_l19));
				vib->vreg_l19 = NULL;
			}
		}
	}
	...
	if (vibrator_gpio_init(vib) < 0) {
		ERR_MSG("[sm100] Android Vreg, GPIO set failed\n");
		return -ENODEV;
	}
