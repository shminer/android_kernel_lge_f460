...
extern VibeStatus ImmVibeSPI_ForceOut_SetSamples(VibeUInt8 nActuatorIndex,
...
static long touch_fops_ioctl(struct file *filp,
...
static ssize_t touch_fops_write(struct file *file,
	...
	if(buf!=NULL)
	{
		...
		if(force_mode == true)
		{
			...
			for(i=0; i<MAX_FORCE_TIME; i+=5)
			{
				...
				ImmVibeSPI_ForceOut_SetSamples(motor_index,
						8, 1, cZero);
				mdelay(5);
			}
