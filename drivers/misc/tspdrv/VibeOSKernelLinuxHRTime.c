 ...
 ** of timer (such as HW timer or standard software timer)
    might be used to achieve
 ** the 5ms required rate.
 */

#ifndef CONFIG_HIGH_RES_TIMERS
#warning "The Kernel does not have high resolution timers enabled. "
"Either provide a non hr-timer implementation of VibeOSKernelLinuxTime.c or "
"re-compile your kernel with CONFIG_HIGH_RES_TIMERS=y"
#endif
...
static enum hrtimer_restart VibeOSKernelTimerProc(struct hrtimer *timer)
{
	...
	{
		/* Do not call SPI functions in this function
		   as their implementation coud use interrupt */
...
static void VibeOSKernelLinuxInitTimer(void)
{
	...
	/* Initialize a 5ms-timer with VibeOSKernelTimerProc
...
static void VibeOSKernelLinuxStartTimer(void)
{
	...
	else
	{
		 ...
		 ** (try to not being stuck here
		 if the semaphore is not freed for any reason)
		 */
		/* wait for the semaphore to be freed by the timer */
		res = down_interruptible(&g_hSemaphore);
		if (res != 0)
		{
			DbgOut((DBL_INFO, "VibeOSKernelLinuxStartTimer: "
						"down_interruptible "
	...
	/*
	 ** Because of possible NACK handling,
	    the  VibeOSKernelProcessData() call above could take more than
	 ** 5 ms on some piezo devices that are buffering output samples;
	    when this happens, the timer
	 ** interrupt will release the g_hSemaphore
	 while VibeOSKernelProcessData is executing and the player
	 ** will immediately send the new packet to the SPI layer
	 when VibeOSKernelProcessData exits, which
	 ** could cause another NACK right away. To avoid that,
	 we'll create a small delay if the semaphore
	 ** was released when VibeOSKernelProcessData exits,
	 by acquiring the mutex again and waiting for
