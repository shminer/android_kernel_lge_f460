...
#include <linux/mfd/max77819-vibrator.h>
...
{
	struct max77819_vib *max77819_vib = container_of(dev,
			struct max77819_vib, timed_dev);
	...
	if (value > 0) {
		...
		hrtimer_start(&max77819_vib->timer, ktime_set(value / 1000,
					(value % 1000) * 1000000),
				HRTIMER_MODE_REL);
	}
}
...
static int max77819_vib_hw_init(struct max77819_vib *max77819_vib,
...
#ifdef CONFIG_OF
static struct
max77819_vib_platform_data *max77819_vib_parse_dt(struct device *dev)
{
...
static int max77819_vib_probe(struct platform_device *pdev)
{
	...
	max77819_vib = devm_kzalloc(dev, sizeof(struct max77819_vib),
			GFP_KERNEL);
	if (unlikely(!max77819_vib))
		return -ENOMEM;

	max77819_vib->regmap = io->regmap;
...
static int max77819_vib_remove(struct platform_device *pdev)
{
