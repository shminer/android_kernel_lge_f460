				...
				void *data)
{
	...
	case CPUFREQ_ADJUST:
		...
		pr_debug("cpu-boost: CPU%u policy min before boost: %u kHz\n",
			 cpu, policy->min);
		pr_debug("cpu-boost: CPU%u boost min: %u kHz\n", cpu, min);

		cpufreq_verify_within_limits(policy, min, UINT_MAX);

		pr_debug("cpu-boost: CPU%u policy min after boost: %u kHz\n",
			 cpu, policy->min);
		break;
...
static void do_boost_rem(struct work_struct *work)
{
	...
	pr_debug("cpu-boost: Removing boost for CPU%d\n", s->cpu);
	s->boost_min = 0;
...
static void update_policy_online(void)
{
	...
	for_each_online_cpu(i) {
		pr_debug("cpu-boost: Updating policy for CPU%d\n", i);
...
static void do_input_boost_rem(struct work_struct *work)
{
	...
	pr_debug("cpu-boost: Resetting input boost min for all CPUs\n");
	for_each_possible_cpu(i) {
...
static int boost_mig_sync_thread(void *data)
{
	...
	while (1) {
		...
		if (req_freq <= dest_policy.cpuinfo.min_freq) {
			pr_debug("cpu-boost: No sync. Sync Freq:%u\n", req_freq);
			continue;
		}
				...
				unsigned long unused, void *arg)
{
	...
	pr_debug("cpu-boost: Migration: CPU%d --> CPU%d\n", mnd->src_cpu, mnd->dest_cpu);
	spin_lock_irqsave(&s->lock, flags);
	s->pending = true;
...
static void do_input_boost(struct work_struct *work)
{
	...
	pr_debug("cpu-boost: Setting input boost min for all CPUs\n");
	for_each_possible_cpu(i) {
		i_sync_info = &per_cpu(sync_info, i);
