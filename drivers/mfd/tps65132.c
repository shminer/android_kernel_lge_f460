...
int tps65132_regulate_voltage(int on, int mode)
{
	...
	if (on) {
		ret = tps65132_write_byte(tps65132_base,
				mode, TPS65132_VREG_LV_40);
		if (ret < 0)
			return ret;
	} else {
		ret = tps65132_write_byte(tps65132_base,
				mode, TPS65132_VREG_LV_50);
		if (ret < 0)
			return ret;
	}
...
static int tps65132_probe(struct i2c_client *cl,
		const struct i2c_device_id *id)
{
	...
	rc = mfd_add_devices(dev, -1, tps65132_devs,
			ARRAY_SIZE(tps65132_devs), NULL, 0, NULL);
	if (rc)
