...
#define HSD_DBG(fmt, args...) \
	pr_info("HSD.max1462x[%-18s:%5d]" \
...
static void button_pressed(struct work_struct *work)
{
	struct delayed_work *dwork = container_of(work, struct delayed_work,
						  work);
	struct hsd_info *hi = container_of(dwork, struct hsd_info,
	...
	if (hi->gpio_get_value_func(hi->gpio_detect) ||
		(atomic_read(&hi->is_3_pole_or_not))) {
		HSD_ERR("4 pole earjack is plugged out already!"
	...
	if (rc < 0) {
		if (rc == -ETIMEDOUT)
			pr_err("adc read timeout\n");
		else
			pr_err("adc read error(%d)\n", rc);
	}
...
static void button_released(struct work_struct *work)
{
	...
	if (hi->gpio_get_value_func(hi->gpio_detect) &&
		...
		HSD_ERR("Earjack is plugged out already!"
			"just ignore the event.\n");
		return;
	}
...
static void max1462x_parse_dt(struct device *dev,
			      struct max1462x_platform_data *pdata)
{
	struct device_node *np = dev->of_node;

	pdata->gpio_detect = of_get_named_gpio_flags(np, "max1462x,gpio_detect",
						     0, NULL);
	pdata->gpio_key = of_get_named_gpio_flags(np, "max1462x,gpio_key",
						  0, NULL);
	pdata->gpio_mic_en = of_get_named_gpio_flags(np, "max1462x,gpio_mic_en",
						     0, NULL);
	pdata->key_code = 0;
...
static int lge_hsd_probe(struct platform_device *pdev)
{
	...
	if (ret < 0) {
		HSD_ERR("Failed gpio_request: gpio%d (gpio_mic_en)\n",
				hi->gpio_mic_en);
		goto error_02;
	}
	...
	if (ret < 0) {
		HSD_ERR("Failed gpio_direction_input: gpio%d (gpio_mic_en)\n",
				hi->gpio_mic_en);
		goto error_02;
	}
	...
	if (ret < 0) {
		HSD_ERR("Failed to gpio_request gpio%d (gpio_det)\n",
				hi->gpio_detect);
		goto error_03;
	}
	...
	if (ret < 0) {
		HSD_ERR("Failed gpio_direction_input: gpio%d (gpio_det)\n",
				hi->gpio_detect);
		goto error_03;
	}
	...
	if (ret < 0) {
		HSD_ERR("Failed gpio_request: gpio%d (gpio_key)\n",
				hi->gpio_key);
		goto error_04;
	}
	...
	if (ret < 0) {
		HSD_ERR("Failed gpio_direction_input: gpio%d (gpio_key)\n",
				hi->gpio_key);
		goto error_04;
	}
	...
	ret = request_threaded_irq(hi->irq_key, NULL, button_irq_handler,
			IRQF_TRIGGER_RISING|IRQF_TRIGGER_FALLING|IRQF_ONESHOT, pdev->name, hi);
	if (ret) {
