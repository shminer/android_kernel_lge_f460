...
/*interrupt size + sizeof(TDMB_BB_HEADER_TYPE)*/
#define TDMB_MPI_BUF_SIZE			(188*16*4 + 8)
#define TDMB_MPI_BUF_CHUNK_NUM		10

static uint8	*gpMPI_Buffer;
static uint8	gBBBuffer_ridx;
static uint8	gBBBuffer_widx;
...
int broadcast_fc8080_drv_if_set_channel(
...
int broadcast_fc8080_drv_if_get_sig_info(
...
int broadcast_fc8080_drv_if_get_msc(
	char** buffer_ptr, unsigned int* buffer_size, unsigned int user_buffer_size)
{
	...
	*buffer_size = tdmb_real_read_size[gBBBuffer_ridx];
	...
	/*printk(
	"broadcast_tdmb_get_dmb_data, read_size %d, total ridx %d, widx %d\n",
