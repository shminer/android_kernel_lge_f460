...
int tdmb_fc8080_mdelay(int32 ms)
{
	...
	if (ms > 100) {
		wait_loop = (ms / 100);
		/* 100, 200, 300 more only ,
		Otherwise this must be modified e.g (ms + 40)/50 */
		wait_ms = 100;
	}
...
static int tdmb_configure_gpios(void)
{
	...
	fc8080_ctrl_info.dmb_en = of_get_named_gpio(
					fc8080_ctrl_info.pdev->dev.of_node,
					"tdmb-fc8080,en-gpio",0);
	...
	fc8080_ctrl_info.dmb_irq = of_get_named_gpio(
					fc8080_ctrl_info.pdev->dev.of_node,
					"tdmb-fc8080,irq-gpio",0);
...
static int __exit broadcast_tdmb_fc8080_remove(struct spi_device *spi)
{
...
static int broadcast_tdmb_fc8080_check_chip_id(void)
{
	...
	/*rc == TRUE : power on success,  OK : 0*/
	if(rc == TRUE && !bbm_com_probe(NULL))
		rc = OK;
	else
		rc = ERROR;
