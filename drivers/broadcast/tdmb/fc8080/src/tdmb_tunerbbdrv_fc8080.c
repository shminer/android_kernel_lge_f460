...
/* Frequency Gap-Interval between Other Ensemble */
#define	ENS_GAP_FREQ		6000
/* Channel Center frequency interval between channel number */
#define	CH_GAP_FREQ		1728
/* Korea TDMB Ensemble Number 7 ~ 13 */
...
extern int tunerbb_drv_fc8080_fic_cb(
	uint32 userdata, uint8 *data, int length);
extern int tunerbb_drv_fc8080_msc_cb(
	uint32 userdata, uint8 subChId, uint8 *data, int length);
...
/*       */
	...
	static int32 gKOREnsembleFullFreqTbl[MAX_KOREABAND_FULL_CHANNEL][2] = {
		{71, 175280}, {72, 177008}, {73, 178736},
		{81, 181280}, {82, 183008}, {83, 184736},
		{91, 187280}, {92, 189008}, {93, 190736},
		{101, 193280}, {102, 195008}, {103, 196736},
		{111, 199280}, {112, 201008}, {113, 202736},
		{121, 205280}, {122, 207008}, {123, 208736},
		{131, 211280}, {132, 213008}, {133, 214736}
	};
#endif

static uint16 is_tdmb_probe;

/*static uint16 data_sequence_count = 0;*/

/*                                                          */
static uint32	antBuffIdx;
static uint16	antBuff[MAX_ANT_BUFF_CNT];
static uint8	calAntLevel;
static uint8	syncLockCnt;
/*                                                          */
/*============================================================
...
#else
void tunerbb_drv_fc8080_get_dm(fci_u32 *mscber,
	fci_u32 *tp_err, fci_u16 *tpcnt, fci_u32 *vaber);
#endif
...
int8 tunerbb_drv_fc8080_set_channel(
	int32 freq_num, uint8 subch_id, uint8 op_mode)
{
	int8 ret_val;
...
int tunerbb_drv_fc8080_fic_cb(uint32 userdata, uint8 *data, int length)
{
	...
	/* FC8000 관련 code인 send_fic_int_sig_isr2task() in mbs_dshmain.c를
	빼다 보니, 현재는 polling 방식이나 향후 ISR방식으로 적용시
	필요하므로 feature를 추가함*/
...
int8	tunerbb_drv_fc8080_get_ber(
	struct broadcast_tdmb_sig_info *dmb_bb_info)
{
...
#ifdef FEATURE_RSSI_DEBUG
	tunerbb_drv_fc8080_get_dm(
		&dmb_bb_info->msc_ber,
		&tp_err_cnt, &nframe, &dmb_bb_info->va_ber, &rssi);
#else
	tunerbb_drv_fc8080_get_dm(
		&dmb_bb_info->msc_ber,
		&tp_err_cnt, &nframe, &dmb_bb_info->va_ber);
#endif
	...
	if (fc8080_serviceType[0] == FC8080_DMB || fc8080_serviceType[0] == FC8080_VISUAL) {
		/*tp_err_cnt = tunerbb_drv_fc8080_rserror_count(&nframe);*/
		/*실시간 frame수 체크*/
...
#if 0
	/*채널은 잡았으나 (sync_status == 0x3f) frame이
	들어오지 않는 경우(nframe == 0) - MBN V-Radio*/
...
/*-------------------------------------------------------------------------------------
int8 tunerbb_drv_fc8080_mulit_set_channel(
	int32 freq_num, uint8 subch_cnt, uint8 subch_id[ ], uint8 op_mode[ ])
...
int8	tunerbb_drv_fc8080_multi_set_channel(
	int32 freq_num, uint8 subch_cnt, uint8 subch_id[], uint8 op_mode[])
{
	int8 res = BBM_OK;
	int32 freq = 0;
...
/*                                                                                     
                                                                                        ...
                                                                                        */
#ifdef STREAM_TS_UPLOAD
int8	tunerbb_drv_fc8080_process_multi_data(
	uint8 subch_cnt, uint8 *input_buf, uint32 input_size, uint32 *read_size)
{
...
#ifdef STREAM_TS_UPLOAD
/*-------------------------------------------------------------------------------------
int8 tunerbb_drv_fc8080_get_multi_data(
	uint8 subch_cnt, uint8* buf_ptr, uint32 buf_size)
...
void tunerbb_drv_fc8080_process_polling_data()
{
	...
	if (mfIntStatus & 0x0100) {
		...
		if (size-1) {
			bbm_com_data(
				hDevice, BBM_COM_FIC_DATA, &ficBuffer[0], size);

			if (pFicCallback) {
			      (*pFicCallback)(
				gFicUserData, &ficBuffer[0], size);
			}
		}
	}
	bbm_com_word_read(hDevice, BBM_BUF_INT, 0x00ff);
