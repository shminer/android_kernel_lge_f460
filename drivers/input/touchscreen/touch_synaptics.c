		...
		const struct touch_data *prev_data)
{
	...
	DO_SAFE(touch_i2c_read(client, 0x04, 1, &buf1), error);
	DO_SAFE(touch_i2c_read(client, 0x05, 1, &buf2), error);
	im = (buf2 << 8) | buf1;
	im_sum += im;

	DO_SAFE(touch_i2c_read(client, 0x08, 1, &cns), error);
	cns_sum += cns;

	DO_SAFE(touch_i2c_read(client, 0x09, 2, buffer), error);
	cid_im = (buffer[1]<<8)|buffer[0];
	cid_im_sum += cid_im;

	DO_SAFE(touch_i2c_read(client, 0x0A, 2, buffer), error);
