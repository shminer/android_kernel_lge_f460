...
#ifdef CONFIG_LGE_PM_CHARGING_LLK_MODE
...
#endif
	bool batt_present;
#if defined(CONFIG_LGE_PM_CHARGING_LLK_MODE)
...
#ifdef CONFIG_LGE_PM_CHARGING_AUDIO_RESTRICT
bool audio_restrict_charging_enabled;
#endif
...
#ifdef CONFIG_LGE_PM_CHARGING_AUDIO_RESTRICT
#define INPUT_CURRENT_LIMIT_AUDIO_RESTRICT 500
#endif
#if defined(CONFIG_LGE_PM_CHARGING_UNIFIED_WLC)
/* G3 HW requirement */
...
#ifdef CONFIG_LGE_PM_THERMALE_CHG_CONTROL
#ifdef CONFIG_LGE_PM_CHARGING_AUDIO_RESTRICT
	if (audio_restrict_charging_enabled == false) {
#endif
		...
		ma = INPUT_CURRENT_LIMIT_AUDIO_RESTRICT;
		pr_info("audio restrict charging enabled : IUSB limit = %d\n", ma);
	}
#endif
#endif
...
#ifndef CONFIG_LGE_PM_CHARGING_LLK_MODE
...
{
#ifdef CONFIG_LGE_PM_BATTERY_MAX17048_FUELGAUGE
	int voltage = 0;
	voltage = max17048_get_voltage() * 1000;
	return voltage;
#else
	pr_err("CONFIG_LGE_PM_BATTERY_MAX17048_FUELGAUGE is not defined.\n");
...
#ifdef CONFIG_LGE_PM_CHARGING_AUDIO_RESTRICT
void set_audio_restrict_charging_enabled(bool enabled)
{
	struct bq24296_chip *chip = the_chip;
	audio_restrict_charging_enabled = enabled;
	power_supply_changed(chip->psy_this);
}
EXPORT_SYMBOL(set_audio_restrict_charging_enabled);
#endif
...
#if defined(CONFIG_LGE_PM_BATTERY_ID_CHECKER)
	uint *smem_batt = 0;
#if defined(CONFIG_LGE_PM_BATTERY_LOW_BATT_LIMIT)
	uint _smem_batt_ = 0;
#endif
#endif
	unsigned int *p_cable_type = (unsigned int *)
...
#ifdef CONFIG_LGE_PM_CHARGING_AUDIO_RESTRICT
	audio_restrict_charging_enabled = false;
#endif
