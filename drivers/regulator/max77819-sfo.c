...
#include <linux/mfd/max77819-sfo.h>
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(3, 5, 0)
static int max77819_list_voltage(struct regulator_dev *rdev,
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 5, 0)
	.volt_table	= max77819_sfo_volt_table,
...
#ifdef CONFIG_OF
static struct
max77819_sfo_platform_data *max77819_reg_parse_dt(struct device *dev)
{
...
static int max77819_sfo_probe(struct platform_device *pdev)
{
	...
	max77819_sfo = devm_kzalloc(dev, sizeof(struct max77819_sfo),
			GFP_KERNEL);
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(3, 5, 0)
	max77819_sfo->regmap = io->regmap;
#endif
	platform_set_drvdata(pdev, max77819_sfo);

	ret = regmap_update_bits(io->regmap, MAX77819_SAFEOUTCTRL,
			MAX77819_ACTDISSAFE, pdata->active_discharge
			? MAX77819_ACTDISSAFE : 0);
	if (IS_ERR_VALUE(ret))
		return ret;
...
#endif
	if (IS_ERR(rdev)) {
		dev_err(dev, "regulator register failed to register regulator "
				": %ld\n", PTR_ERR(rdev));
		return PTR_ERR(rdev);
	}
...
static int max77819_sfo_remove(struct platform_device *pdev)
{
