...
#define NFC_IRQ_ENABLED	true
#define NFC_IRQ_DISABLED	false
static bool sIrqState;
#endif


#ifdef LGE_NFC_READ_IRQ_MODIFY
bool do_reading; /* DY_TEST */
static bool cancle_read; /* DY_TEST */
#endif
...
static void pn544_parse_dt(struct device *dev, struct pn544_dev *pn544_dev)
{
	...
	/* irq gpio info */
	pn544_dev->ven_gpio =
		of_get_named_gpio_flags(np, "NXP,gpio_ven", 0, NULL);
	pn544_dev->firm_gpio =
		of_get_named_gpio_flags(np, "NXP,gpio_mode", 0, NULL);
	pn544_dev->irq_gpio =
		of_get_named_gpio_flags(np, "NXP,gpio_irq", 0, NULL);
}
...
void pn544_factory_standby_set(void)
{
	...
	uint8_t EEDATA_WRITE[9] =
		{0x08, 0x00, 0x06, 0x00, 0x9E, 0xAA, 0x00, 0x01, 0x01};
...
static char pn544_standby_set_val3[10] = {0x09, 0x89, 0x83, 0x3F, 0x00, 0x9E,
					  0xAA, 0x01, 0xC2, 0x85};
...
static int __pn544_kwrite(void *dev, void *data, int size)
{
	...
	while (retry != NFC_I2C_WRITE_RETRY_NUM) {
		...
		ret = i2c_master_send(pn544_client, data, size);
		retry++;
		pr_info("%s i2c_master_send retry[%d]\n", __func__, retry);
	}
...
static int __pn544_kread(void *dev, unsigned int length)
{
	...
	mutex_unlock(&mode_mutex);
	pr_info("%s: read data : 0x%X 0x%X 0x%X 0x%X\n", __func__,
...
void pn544_factory_standby_set(void)
{
	...
	if (ret == 0) {
		pr_info("%s: standby write val1 success\n", __func__);
		ret = __pn544_kread(pn544_dev, 4);
	} else {
		pr_err("%s: standby write val1 fail\n", __func__);
		return;
	}

	ret = __pn544_kwrite(pn544_dev, pn544_standby_set_val2, 6);
	if (ret == 0) {
		pr_info("%s: standby write val2 success\n", __func__);
		ret = __pn544_kread(pn544_dev, 6);
	} else {
		pr_err("%s: standby write val2 fail\n", __func__);
		return;
	}

	ret = __pn544_kwrite(pn544_dev, pn544_standby_set_val3, 10);
	if (ret == 0) {
		pr_info("%s: standby write val3 success\n", __func__);
		ret = __pn544_kread(pn544_dev, 7);
	} else {
		pr_err("%s: standby write val3 fail\n", __func__);
		return;
	}
 ...
 */
#if defined(CONFIG_LGE_NFC_MULTICORE_FASTBOOT) && \
	defined(CONFIG_LGE_NFC_PRESTANDBY)
...
#endif /* defined(CONFIG_LGE_NFC_MULTICORE_FASTBOOT) && \
	defined(CONFIG_LGE_NFC_PRESTANDBY) */
/* LGE_CHANGE_E */
	...
	if (!stReadIntFlag) {
		...
		if (irq_gpio_val == 0) {
			if (filp->f_flags & O_NONBLOCK) {
				pr_err("%s:f_falg has O_NONBLOCK. EAGAIN!\n",
					 __func__);
				ret = -EAGAIN;
				goto fail;
			}
...
/*	20120831, jh.heo@lge.com Fix to irq interrupt in sleep mode. */
		...
		ret = wait_event_interruptible(pn544_dev->read_wq, do_reading);
			...
			pn544_disable_irq(pn544_dev);
			/*
			dprintk("%s:wait_event_interruptible : %d\n",
				 __func__,  ret);
			*/
#ifdef LGE_NFC_READ_IRQ_MODIFY
		/* DY_TEST */
	 ...
	 unsigned long arg)
{
	...
	case PN544_SET_PWR:
		if (arg == 2) {
			...
			*/
			...
			msleep(10);
#ifdef CONFIG_LGE_NFC_SET_IRQ_WAKEUP
			if (sIrqState == NFC_IRQ_DISABLED) {
				irq_set_irq_wake(pn544_dev->client->irq, 1);
				sIrqState = NFC_IRQ_ENABLED;
					dprintk("%s: enable IRQ\n", __func__);
			} else {
			...
			msleep(10);
#ifdef CONFIG_LGE_NFC_SET_IRQ_WAKEUP
			if (sIrqState == NFC_IRQ_ENABLED) {
				irq_set_irq_wake(
					pn547_dev->client->irq, 0);
				sIrqState = NFC_IRQ_DISABLED;
	...
	case PN544_INTERRUPT_CMD:
		{
			...
			*/
			dprintk("%s: pn544_interrupt enable level:%ld\n",
				__func__, arg);
			break;
		}
	case PN544_READ_POLLING_CMD:
		{
			stReadIntFlag = arg;
			dprintk("%s: pn544_polling flag set:%ld\n",
				__func__, arg);
			break;
		}
		...
		const struct i2c_device_id *id)
{
	...
	if (ret) {
		dprintk("%s: nfc_int request failed!\n", __func__);
		goto err_int;
	}
	ret = gpio_request(pn544_dev->ven_gpio, "nfc_ven");
	if (ret) {
		dprintk("%s: nfc_ven request failed!\n", __func__);
		goto err_ven;
	}
	ret = gpio_request(pn544_dev->firm_gpio, "nfc_firm");
	if (ret) {
		dprintk("%s: nfc_firm request failed!\n", __func__);
		goto err_firm;
	}
 ...
 */
	...
	if (pn544_validate_boot_mode()) {
...
#ifdef CONFIG_LGE_NFC_MULTICORE_FASTBOOT
		{
			...
			th = kthread_create(pn544_factory_standby_set_thread,
						 NULL, "pn544_factory_standby");
			if (IS_ERR(th)) {
				ret = PTR_ERR(th);
				goto err_request_irq_failed;
			}
