...
wait:
	...
	/*
	dprintk("%s: i2c_master_recv success (%d)\n", __func__, ret);
	*/
	return ret;
}
		...
		const struct i2c_device_id *id)
{
	...
	if (ret) {
		pr_err("%s: nfc_int request failed!\n", __func__);
		goto err_int;
	}
	ret = gpio_request(pn547_dev->ven_gpio, "nfc_ven");
	if (ret) {
		pr_err("%s: nfc_ven request failed!\n", __func__);
		goto err_ven;
	}
	ret = gpio_request(pn547_dev->firm_gpio, "nfc_firm");
	if (ret) {
		pr_err("%s: nfc_firm request failed!\n", __func__);
		goto err_firm;
	}
	...
	pn547_dev->irq_enabled = true;
	ret = request_irq(pn547_gpio_to_irq(pn547_dev), pn547_dev_irq_handler, IRQF_TRIGGER_RISING|IRQF_NO_SUSPEND, client->name, pn547_dev);
	if (ret) {
