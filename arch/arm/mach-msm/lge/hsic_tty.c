/* arch/arm/mach-msm/lge/hsic_tty.c
 *
...
struct hsic_tty_info {
	...
	unsigned int (*send_carrier_detect)(struct hsic_tty_info *p,
			unsigned int);
	unsigned int (*send_ring_indicator)(struct hsic_tty_info *p,
			unsigned int);
...
#ifdef CONFIG_MODEM_SUPPORT
static int hsic_tty_tiocmset(struct tty_struct *tty,
		unsigned int set,
		unsigned int clear);
...
unsigned int hsic_tty_send_carrier_detect(struct hsic_tty_info *info,
		unsigned int yes)
{
	u16         state;
...
unsigned int hsic_tty_send_ring_indicator(struct hsic_tty_info *info,
		unsigned int yes)
{
	u16         state;
...
static int hsic_tty_send_break(struct hsic_tty_info *info,
		int duration)
{
	u16         state;
...
static int hsic_tty_send_modem_ctrl_bits(struct hsic_tty_info *info,
		int ctrl_bits)
{
...
static int hsic_tty_write(struct tty_struct *tty,
...
{
	struct hsic_tty_info *info = tty->driver_data;
	/* u16 bits = info->port_handshake_bits; */
	u16 state = info->serial_state;
	return
		/* ((bits & ACM_CTRL_RTS) ? TIOCM_RTS : 0) | */
		/* ((bits & ACM_CTRL_DTR) ? TIOCM_DTR : 0) | */
		/* ((bits & ACM_CTRL_RTS) ? TIOCM_CTS : 0) | */
		((state & ACM_CTRL_DSR) ? TIOCM_DSR : 0) |
		((state & ACM_CTRL_DCD) ? TIOCM_CD : 0) |
		((state & ACM_CTRL_RI) ? TIOCM_RI : 0);
}
		...
		unsigned int clear)
{
	...
	info->port_handshake_bits |=
		((set & TIOCM_DTR) ? ACM_CTRL_DTR : 0) |
		((set & TIOCM_RTS) ? ACM_CTRL_RTS : 0);
	/* clear */
	info->port_handshake_bits &=
		~(((clear & TIOCM_DTR) ? ACM_CTRL_DTR : 0) |
	...
	if (info->port_handshake_bits & (ACM_CTRL_DTR | ACM_CTRL_RTS)) {
		pr_debug("%s: ACM_CTRL_DTR and ACM_CTRL_RTS is set."
				" start write_tomdm\n", __func__);
		spin_lock(&port->rx_lock);
		queue_work(port->wq, &port->write_tomdm_w);
		spin_unlock(&port->rx_lock);
...
const struct tty_operations hsic_tty_ops = {
	.open = hsic_tty_open,
...
static int __init hsic_tty_init(void)
{
	...
	for (n = 0; n < MAX_HSIC_TTYS; ++n) {
		pr_info("%s: %d\n", __func__, n);
		tty_port_init(&hsic_tty.tport);
		tty_dev = tty_port_register_device(&hsic_tty.tport,
				hsic_tty_driver, n, NULL);

		if (IS_ERR(tty_dev)) {
			pr_err("%s: failed to register tty for port %d, err %ld\n",
					__func__, n, PTR_ERR(tty_dev));
			goto out;
		}
	}
	return 0;
