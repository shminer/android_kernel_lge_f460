...
const const char *data_bridge_names[] = {
	"serial_hsic_data",
	"rmnet_hsic_data"
};
...
static unsigned int
hsic_tty_data_rmnet_tx_q_size = HSIC_TTY_DATA_RMNET_TX_Q_SIZE;
module_param(hsic_tty_data_rmnet_tx_q_size, uint, S_IRUGO | S_IWUSR);

static unsigned int
hsic_tty_data_rmnet_rx_q_size = HSIC_TTY_DATA_RMNET_RX_Q_SIZE;
module_param(hsic_tty_data_rmnet_rx_q_size, uint, S_IRUGO | S_IWUSR);

static unsigned int
hsic_tty_data_serial_tx_q_size = HSIC_TTY_DATA_SERIAL_TX_Q_SIZE;
module_param(hsic_tty_data_serial_tx_q_size, uint, S_IRUGO | S_IWUSR);

static unsigned int
hsic_tty_data_serial_rx_q_size = HSIC_TTY_DATA_SERIAL_RX_Q_SIZE;
module_param(hsic_tty_data_serial_rx_q_size, uint, S_IRUGO | S_IWUSR);

static unsigned int
hsic_tty_data_rx_req_size = HSIC_TTY_DATA_RX_REQ_SIZE;
...
static unsigned int
hsic_tty_data_fctrl_support = HSIC_TTY_DATA_FLOW_CTRL_SUPPORT;
module_param(hsic_tty_data_fctrl_support, uint, S_IRUGO | S_IWUSR);

static unsigned int
hsic_tty_data_fctrl_en_thld = HSIC_TTY_DATA_FLOW_CTRL_EN_THRESHOLD;
module_param(hsic_tty_data_fctrl_en_thld, uint, S_IRUGO | S_IWUSR);

static unsigned int
hsic_tty_data_fctrl_dis_thld = HSIC_TTY_DATA_FLOW_CTRL_DISABLE;
module_param(hsic_tty_data_fctrl_dis_thld, uint, S_IRUGO | S_IWUSR);

static unsigned int
hsic_tty_data_pend_limit_with_bridge = HSIC_TTY_DATA_PENDLIMIT_WITH_BRIDGE;
...
#if 0
static void hsic_tty_data_free_requests(struct usb_ep *ep,
...
static int hsic_tty_data_alloc_requests(struct usb_ep *ep,
		struct list_head *head,
		int num,
...
static void hsic_tty_data_write_tohost(struct work_struct *w)
{
	...
	for (;;) {
		if (is_in_reset(info)) {
			...
			tty_insert_flip_char(tport, 0x00, TTY_BREAK);
...
#ifdef VERBOSE_DEBUG
		print_hex_dump(KERN_DEBUG, "tohost:", DUMP_PREFIX_OFFSET,
				16, 1, skb->data, skb->len, 1);
#endif
		ret = tty_prepare_flip_string(tport, &ptr, skb->len);
		if (ret <= 0) {
	...
	if (do_tty_wake && tport->tty)
		tty_wakeup(tport->tty);
	mutex_unlock(&hsic_tty_lock);
}
...
static void hsic_tty_data_write_tomdm(struct work_struct *w)
{
	...
	if (!port || !atomic_read(&port->connected)) {
		pr_debug("%s: port(%p), connected(%d)\n", __func__,
				port, atomic_read(&port->connected));
		return;
	}
...
#ifdef VERBOSE_DEBUG
		print_hex_dump(KERN_DEBUG, "tomdm:", DUMP_PREFIX_OFFSET,
				16, 1, skb->data, skb->len, 1);
#endif
...
#if 0
static void hsic_tty_data_epin_complete(struct usb_ep *ep,
		struct usb_request *req)
{
...
static void hsic_tty_data_start_io(struct tdata_port *port)
{
	...
	ret = hsic_tty_data_alloc_requests(ep,
			&port->rx_idle,
			port->rx_q_size,
			hsic_tty_data_epout_complete,
			GFP_ATOMIC);
	if (ret) {
