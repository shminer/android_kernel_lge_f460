...
#if CONFIG_LGE_PM_CHARGING_AUDIO_RESTRICT
/* restrict charging current */
static int restrict_charging;
#endif
...
#ifdef CONFIG_MACH_LGE /* LGE added samplerate */
...
#if CONFIG_LGE_PM_CHARGING_AUDIO_RESTRICT
static int restrict_charging_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = restrict_charging;
	return 0;
}

extern void set_audio_restrict_charging_enabled(bool);

static int restrict_charging_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	//char *current_max = "500";

	restrict_charging = ucontrol->value.integer.value[0];
	pr_info("%s: restrict_charging:%d\n", __func__, restrict_charging);
	/* TODO : control thermal-engine function */
	if (restrict_charging) {
		/* save input_current_max value for restore */
		pr_debug("%s: enable\n", __func__);
	} else {
		/* restore current_max value */
		pr_debug("%s: disable\n", __func__);
		//current_max = current_max_save;
	}
    set_audio_restrict_charging_enabled(restrict_charging);
...
#endif /*CONFIG_SND_SOC_CS35L32*/
#if CONFIG_LGE_PM_CHARGING_AUDIO_RESTRICT
	SOC_SINGLE_EXT("Restrict Charging", SND_SOC_NOPM, 0, 1, 0,
			restrict_charging_get, restrict_charging_put),
#endif
};
